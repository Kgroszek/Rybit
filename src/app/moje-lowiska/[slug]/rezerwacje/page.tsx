...
import { OwnerReservationsBoard } from "@/components/owner/OwnerReservationsBoard";
...
export default async function OwnerReservationsPage({
  ...
  const toDateKey = addDaysToDateKey(from, days);
  const rangeStart = warsawDateTimeToUtc(from, "00:00");
  const rangeEnd = warsawDateTimeToUtc(toDateKey, "00:00");

  if (!rangeStart || !rangeEnd) {
    redirect(`/moje-lowiska/${slug}/rezerwacje`);
  }
  ...
  const now = new Date();
  const activeNow = await prisma.lakeReservation.count({
    where: {
      lakeId: lake.id,
  ...
  const pendingCount = await prisma.lakeReservation.count({
    where: {
      lakeId: lake.id,
      status: "pending",
      endsAt: { gt: now },
      ...
      <div className="mx-auto w-full max-w-[1600px] px-4 pb-28 pt-6 sm:px-6 lg:px-8 lg:pb-10 lg:pt-8">
        ...
        <OwnerReservationsBoard
          ...
          reservations={reservations.map((reservation) => ({
            ...reservation,
            status: reservation.status === "paid" ? "confirmed" : reservation.status,
            startsAt: reservation.startsAt.toISOString(),
            endsAt: reservation.endsAt.toISOString(),
          }))}
          initialNew={query.new === "1"}
          initialSpotId={query.spotId ?? null}
          initialReservationId={query.reservationId ?? null}
        />
      </div>
    </DashboardLayout>
  );
