    ...
    <div className="mt-10 rounded-3xl bg-white/85 p-5 shadow-sm backdrop-blur sm:p-6">
      ...
      <div className="grid min-w-0 grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-6">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-500">
                    Największe znane ryby pływające na tym łowisku.
                  </p>
                </div>

                <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">
                  {lake.recordFish.length} rekordów
                </span>
              </div>

              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {lake.recordFish.map((item) => (
                  <div
                    key={item.id}
          ...
          {(lake.equipmentRequirements?.length ?? 0) > 0 && (
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-xl font-bold text-slate-950">
                Wymagania sprzętowe
              </h2>
              ...
              <p className="mt-1 text-sm leading-6 text-slate-500">
                Sprzęt wymagany lub zalecany przez łowisko.
              </p>

              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {lake.equipmentRequirements.map((item) => (
                  <div
                    key={item}
                    className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4"
                  >
