...
import { createGraphicCatchCardFile } from "@/lib/catch-card-client";
...
export function CatchShareDialog({
  ...
  const graphicTemplate = useMemo(
    () => getCatchCardTemplate(fishingCatch.fishName),
    [fishingCatch.fishName]
  );

  const isGraphicTemplate = Boolean(graphicTemplate);

  const cardUrl = useMemo(() => {
    if (isGraphicTemplate) {
      return null;
    }

    return `/api/catches/${fishingCatch.id}/card?format=${format}&variant=${variant}`;
  }, [fishingCatch.id, format, isGraphicTemplate, variant]);
  ...
  useEffect(() => {
    ...
    async function preparePreview() {
      ...
      try {
        const file = await createGraphicCatchCardFile({
          id: fishingCatch.id,
          fishName: fishingCatch.fishName,
          userName: fishingCatch.userName,
        ...
        setGraphicPreviewUrl(revokedUrl);
      } catch (error) {
        if (!cancelled) {
          setGraphicPreviewUrl(null);
          setMessage(
  ...
  async function getCardFile() {
    if (isGraphicTemplate) {
      return createGraphicCatchCardFile({
        id: fishingCatch.id,
        fishName: fishingCatch.fishName,
        userName: fishingCatch.userName,
        method: fishingCatch.method,
  ...
  return (
    <div
      className="fixed inset-0 z-[1400] flex items-center justify-center bg-navy-950/75 p-3 backdrop-blur-sm sm:p-5"
      onMouseDown={onClose}
    >
      <div
        className="max-h-[94dvh] w-full max-w-6xl overflow-y-auto rounded-modal border border-border bg-surface shadow-2xl"
        onMouseDown={(event) => event.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-label="Karta połowu"
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-border bg-surface/95 p-5 backdrop-blur sm:p-6">
          <div>
            ...
            <h2 className="mt-1 font-display text-2xl font-extrabold tracking-[-0.03em] text-text">
              {fishingCatch.fishName}
            </h2>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-navy-950 px-3 py-1.5 text-sm font-bold text-white">
                Rybio Score {score.score ?? "—"}/100
              </span>
              <span className="rounded-full border border-primary-200 bg-primary-100 px-3 py-1.5 text-xs font-bold text-primary-800">
                {score.tierLabel}
              </span>
            </div>
            <p className="mt-3 max-w-2xl text-xs leading-5 text-text-secondary">
              {score.explanation}
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-surface-muted text-text-secondary transition hover:bg-surface-hover hover:text-text"
            aria-label="Zamknij"
          >
        ...
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="min-w-0">
            {!isGraphicTemplate && (
              <div className="mb-3 grid gap-2 sm:grid-cols-2">
                <SegmentedControl
                  value={variant}
                  options={[
                    { value: "collector", label: "Kolekcjonerska" },
                    { value: "clean", label: "Klasyczna" },
                  ]}
                  onChange={(value) =>
                    setVariant(value as CatchCardVariant)
                  }
                />
                <SegmentedControl
                  value={format}
                  ...
                  onChange={(value) =>
                    setFormat(value as CatchCardFormat)
                  }
                />
              </div>
            )}
            ...
            {isGraphicTemplate ? (
              <div className="mx-auto max-w-[360px] overflow-hidden rounded-card border border-border bg-surface-muted shadow-card">
                {graphicPreviewUrl ? (
                  <img
                  ...
                  <div className="flex aspect-[9/16] items-center justify-center px-6 text-center text-sm font-semibold text-text-secondary">
                    {message || "Przygotowywanie podglądu…"}
                  </div>
                )}
              </div>
            ) : (
              <div
                className={`mx-auto overflow-hidden rounded-card border border-border bg-surface-muted shadow-card ${
                  format === "story"
                    ? "max-w-[360px]"
                    : "max-w-[610px]"
                }`}
          ...
          <aside>
            <div className="rounded-card border border-border bg-surface-muted p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-text-secondary">
                {isGraphicTemplate
                  ? "1080 × 1920 px — dedykowana karta do relacji i Stories."
                  : format === "post"
              ...
              <div className="mt-4 rounded-control border border-primary-200 bg-primary-100 p-4">
                ...
                <p className="mt-2 font-display text-3xl font-extrabold text-primary-950">
                  {score.score ?? "—"}
                  <span className="text-base text-primary-500"> / 100</span>
                </p>
              ...
              {!fishingCatch.isPublic && (
                <div className="mt-5 rounded-control border border-warning-border bg-warning-subtle p-4">
                  ...
                  <p className="mt-1 text-xs leading-5 text-text-secondary">
                    Kartę możesz pobrać lub wysłać jako plik. Publiczny link jest dostępny tylko dla publicznych połowów.
                  </p>
                </div>
              )}

              {message && graphicPreviewUrl && (
                <p className="mt-4 rounded-control bg-surface p-3 text-xs font-semibold leading-5 text-text-secondary">
                  {message}
                </p>
              )}
            </div>
    ...
    <div className="flex rounded-control bg-surface-muted p-1">
      {options.map((option) => (
        <button
          ...
          className={`min-h-9 flex-1 rounded-xl px-3 text-xs font-bold transition ${
            value === option.value
