...
export function RecommendedLakes({
  ...
  const recommendedLakes =
    useMemo<RecommendedLake[]>(() => {
  ...
  return (
    <section>
      <SectionHeader
        eyebrow="Polecane"
        title="Łowiska dla Ciebie"
        description={
          userLocation &&
          isValidLocation(userLocation)
            ? "Wybrane spośród miejsc położonych najbliżej Twojej zapisanej lokalizacji."
        ...
        {recommendedLakes.map((lake) => {
          ...
          return (
            <Link
              ...
              className="group relative min-h-[330px] overflow-hidden rounded-card bg-navy-950 shadow-card transition-[box-shadow,transform] duration-300 hover:-translate-y-0.5 hover:shadow-card-hover"
            >
              {lake.images[0] ? (
                <img
                  src={lake.images[0]}
                ...
                <div className="absolute inset-0 bg-[linear-gradient(145deg,#0D1E33,#18324F)]" />
              )}
                ...
                <span
                  className={
                    lake.type === "commercial"
                      ? "rounded-full border border-white/20 bg-success/90 px-3 py-1 text-[10px] font-extrabold text-white backdrop-blur"
                      : "rounded-full border border-white/20 bg-primary/90 px-3 py-1 text-[10px] font-extrabold text-white backdrop-blur"
                  }
                >
                  {getLakeTypeLabel(lake.type)}
                </span>

                <span className="rounded-full border border-white/20 bg-white/90 px-3 py-1 text-[10px] font-extrabold text-navy-950 backdrop-blur">
                  {getFishingTypeLabel(
                    lake.fishingType
                  )}
                </span>
                  ...
                  {lake.calculatedDistance !==
                    ...
                    Number.isFinite(
                      ...
                      <>
                        ...
                        <span>
                          {lake.calculatedDistance.toFixed(
                            1
                          )}{" "}
                          km
                        </span>
